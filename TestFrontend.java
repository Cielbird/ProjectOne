...
public class TestFrontend {
	...
	public void runTests() {
		...
		System.out.print("Test invalid rating on ratings selection screen (WARNING: if pressing 'x' twice from the ratings selection screen does not exit app, test won't exit and run indefinitely!): ");
		if(this.testFrontendForInvalidRating()) {
			System.out.println("PASSED");
		} else {
			System.out.println("FAILED");
		}
	...
	/**
	 * This test runs the front end and redirects its output to a string. It then
	 * passes in 'r' as a command to go to the ratings mode. It then exits
	 * the ratings mode by pressing 'x' to go back to the base mode and another 'x' to exit.
	 * The test succeeds if the ratings screen is successfully displayed and exited.
	 * It fails if the ratings menu is not displayed (so, if one or more of the ratings is missing), 
	 * the front end has not been instantiated (is null), or there is an exception.
	 * @return true if the test passed, false if it failed
	 */
	public boolean testFrontendRForRatings() {
		...
		InputStream standardIn = System.in;
		try {
			// set the input stream to our input (with an r to test the program lists ratings)
			...
			if (frontend != null && appOutput.contains("0")
				...
				// test passes if all the rating options are displayed on the screen
				return true;
			} else {
				// test failed
				return false;
	...
	/**
	 * This test runs the front end and redirects its output to a string. It then
	 * passes in 'r' as a command to go to the ratings mode, followed by '11', which is 
	 * invalid rating that is not listed on the option menu. This should not cause the program to crash. 
	 * It then exits the app by pressing 'x' to go back to the main mode and another 'x' to exit.
	 * The test succeeds if the invalid rating does not cause the program to crash.
	 * It fails if the front end has not been instantiated (is null), or there is an exception/the program 
	 * crashes.
	 * @return true if the test passed, false if it failed
	 */
	public boolean testFrontendForInvalidRating() {
		PrintStream standardOut = System.out;
		InputStream standardIn = System.in;
		try {
			// set the input stream to our input (with an r to test the program lists ratings)
			String input = "r" + System.lineSeparator() + "11" + System.lineSeparator();
			input += "x" + System.lineSeparator() + "x";
			InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
			System.setIn(inputStreamSimulator);
			ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
			// set the output to the stream captor to read the output of the front end
